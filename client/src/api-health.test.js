...
test('descreve API online sem MySQL configurado', () => {
  assert.deepEqual(
    ...
    {
      state: 'success',
      message: 'API online. Fase 9. MySQL ainda não configurado, como esperado nesta etapa.',
    },
  );
});
...
test('distingue API online de MySQL indisponível', () => {
  assert.deepEqual(
    ...
    {
      state: 'warning',
      message: 'API online. Fase 9. O MySQL está indisponível. Verifique a configuração do banco.',
    },
  );
});
