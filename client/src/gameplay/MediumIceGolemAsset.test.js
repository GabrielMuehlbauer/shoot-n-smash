...
async function parseMediumAsset() {
  const bytes = await readFile(
    new URL('../../public/assets/models/ice_golem_medium.glb', import.meta.url),
  );
...
test('GLB do monstro 2 preserva anatomia, rig e orçamento web', async () => {
  const { bytes, gltf: { scene } } = await parseMediumAsset();
  prepareMediumGolemAsset(scene);
...
test('somente o inimigo médio usa o modelo do monstro 2', async () => {
  const { gltf } = await parseMediumAsset();
  const requested = [];
  const loader = {
    async loadAsync(url) {
      requested.push(url);
  ...
  const mediumType = GAMEPLAY_CONFIG.enemy.types[1];
  const enemy = new EnemySystem({
    ...
    mediumAssetLoader: loader,
  });
  assert.equal(enemy.visual.children.length, 17);
  await enemy.mediumAssetPromise;

  assert.deepEqual(requested, [MEDIUM_GOLEM_URL]);
  assert.equal(enemy.visual.children.length, 1);
  assert.equal(enemy.mediumAsset.visible, true);
  assert.equal(
    enemy.mediumAsset.scale.x,
    MEDIUM_GOLEM_GAME_HEIGHT / (MEDIUM_GOLEM_SOURCE_HEIGHT * mediumType.visualScale),
  );

  enemy.reset({
    enemyType: GAMEPLAY_CONFIG.enemy.types[2],
    radius: GAMEPLAY_CONFIG.enemy.radius,
    visualScale: GAMEPLAY_CONFIG.enemy.types[2].visualScale,
    spawnHeight: GAMEPLAY_CONFIG.enemy.spawn.height,
  });
  assert.equal(enemy.mediumAsset.visible, false);
